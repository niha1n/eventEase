...
const ajProtectedPOST = async (req: NextRequest) => {
  ...
  if (isSignUp) {
    try {
      ...
      if (!authResponse.ok) {
        console.log("[Sign-up] Better Auth returned error:", authResponse.status);
        return authResponse;
      }
      ...
      if (authData.user && authData.user.id && role) {
        console.log(`[Sign-up] Updating user ${authData.user.id} role to ${role}`);
